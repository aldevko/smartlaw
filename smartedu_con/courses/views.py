...
def search(request):
    courses = Course.objects.filter(name__contains = request.GET['search'], description__contains = request.GET['search'])#eğer burada başlığa yani name'e göre değil de
    #içeriğe göre arama yapmak istersen şu kodu yaz: courses = Course.objects.filter(description__contains = request.GET['search'])
    categories = Category.objects.all()
    tags = Tag.objects.all()
